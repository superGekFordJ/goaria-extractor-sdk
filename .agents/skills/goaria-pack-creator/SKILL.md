...
- **Zig (`--lang zig`)**: `build.zig`, `build.zig.zon`, `manifest.json`, `src/main.zig`, `.gitignore`, plus `vendor/goaria_sdk/` (verbatim `sdk/zig` copy, `.zig-cache` excluded). The zon declares `.goaria_sdk = .{ .path = "vendor/goaria_sdk" }` and `src/main.zig` uses `@import("goaria_sdk")` + `goaria.exportExtractor(...)`. `--sdk git`/`--sdk crates` fail for zig.
